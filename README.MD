...
### 如何实现前后端分离
&emsp;&emsp;主要是两者之间通过HTTP通信，本项目前端采用的是Vue后端采用的是Spring Boot。前端通过调用后端的API来实现增删改查，前端通过使用fetch()方法来异步获取后端的资源。需要在fetch中指定请求的头和body。后端则提供类似RESTful风格的URL。部署到服务上的时候需要一层Nginx反向代理层
